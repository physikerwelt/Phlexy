...
$lexerDefinition = array(
	'MATHMODE' => array(
		...
		'[0-9a-zA-Z\+\(\)\s\=\-\,\*\:/\.\;\?!\`´\[\]\>\<]+'=> function(Stateful $lexer) {
...
function checktex($tex){
	global $lexer, $validCommands, $debug;
	try{
	...
	$outstr='';
	$duplicateCurlClose = array();
	$duplicateCurlState = -1;
	foreach ($tokens as $key => $value) {
		...
		switch ($type):
			case 'COMMANDNAME':
				if (!in_array($content, $validCommands)){
					if( $debug ){
			...
			case 'SUBSUP-START':
				$duplicateCurlState++;
				$duplicateCurlClose[$duplicateCurlState] = 0;
				$content = "{".$content;
				break;
			case '}':
				if( $duplicateCurlState >= 0 ){
					$duplicateCurlClose[$duplicateCurlState]--;
					if ( $duplicateCurlClose[$duplicateCurlState] == -1 ){
						$content = "}}";
					}	
				}
				break;
